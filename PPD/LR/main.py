...
_path = r'E:\06-Workspace\071-PyPrj\iScore\PPD\LR'
...
folderOfData = 'E:/06-Workspace/071-PyPrj/iScore/PPD/datasets/first round train data/'
    ...
    if not BRM:
        if special_attribute == []:
            ...
            trainData[col1] = trainData[col].map(
                lambda x: sf.AssignBin(x, cutOffPoints, special_attribute=special_attribute))
        else:
            ...
            trainData[col1] = trainData[col].map(
                lambda x: sf.AssignBin(x, cutOffPoints2, special_attribute=special_attribute))
