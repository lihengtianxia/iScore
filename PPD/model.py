...
for tw in time_window:
    ...
    for var in var_list:
        ...
        data1GroupbyIdx[str(var) + '_' + str(tw) + '_avg_count'] = data1GroupbyIdx[
            ...
            apply(lambda x: divided_by_zero(x[0], x[1]), axis=1)
...
'''
data3_train['UserupdateInfo1'] = data3_train['UserupdateInfo1'].map(change_content)
...
allData = pd.concat([data2_train.set_index('Idx'), data3GroupbyIdx.set_index('Idx'), data1GroupbyIdx.set_index('Idx')],
                    axis=1)
...
allData.to_csv(folderOfData + 'allData_1_XGBoost.csv', header=True, encoding='gbk', columns=allData.columns,
               index=False)

allData = pd.read_csv(folderOfData + 'allData_1_XGBoost.csv', header=0, encoding='gbk')
...
best_colsample_bytree, best_subsample = gsearch3.best_params_['colsample_bytree'], gsearch3.best_params_[
    'subsample']  # 0.8, 0.6
